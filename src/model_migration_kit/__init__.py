...
**Why those three are the exception.** The package ships four data files. Three of
them are the demo's -- ``demo_goldenset.jsonl``, ``demo_rubric.md`` and
``demo.toml`` -- and a reader who installed the wheel had no way to reach any of
them. The fourth, ``showcase_goldenset.jsonl``, is the ninety-six-item synthetic
set the published showcase report is seeded from; it has no accessor here on
purpose. Nothing a user runs reads it -- the showcase is generated by
``scripts/`` against a checkout, and what a stranger consumes is the rendered
document, not the seed -- so an accessor would be a public promise made for no
caller. It is in the wheel because it lives inside the package directory and the
build includes that directory wholesale; ``scripts/verify_release.py``'s
``DEMO_DATA`` names only the three above, which is why a fourth file in ``data/``
does not disturb the release gate. The README's own example said
...
#: names. (It also holds ``showcase_goldenset.jsonl``, which is seed data for the
#: published report rather than something a user of this package reaches for; the
#: docstring above says why it gets no name here.) ``demo.py`` imports these
